...
@torch_func
def kmeanspp(K, n_clusters, device, w):
    """
    ...
    # print(w)
    # h: initialization for \barCw
    first_centroid = np.random.choice(n) # a random index
    # TODO: Only the numpy seed is reset n_init times, while torch seed is fixed, still args.seed
    # first_centroid = torch.multinomial(w, 1).item() # a random index
    h[0, first_centroid] = 1 # only specify one sample's cluster, ratio in the cluster is also 1
    for c_idx in range(1, n_clusters):
        h_e = h.unsqueeze(1) - e.unsqueeze(0)  # [k, n, n]
        # torch.min return minvalue and idx
        # distance of n samples to k centroids -> distance to the closest centroids
...
@torch_func
def predict(K, h, max_iter: int, A, device, w, tolerate_empty_cluster=False):
    # h: initialization for \barCw
    ...
    for _ in range(max_iter):
        h_e = h.unsqueeze(1) - e.unsqueeze(0)  # [k, n, n]
        # label of the closest cluster for each point

        # print(h_e.shape, K.shape)
        # sys.exit()
        ...
        # empty cluster! exit with success=False
        if not tolerate_empty_cluster and torch.any(nn == 0): 
            success = False
            break
