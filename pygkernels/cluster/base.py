    ...
    def wrapper(*args, **kwargs):
        with torch.no_grad():
            args = [torch.from_numpy(x).float().to(kwargs['device'])
                    ...
                    for x in args]
            kwargs = {x:torch.from_numpy(kwargs[x]).float().to(kwargs['device'])
                if type(kwargs[x]) in [np.ndarray, np.memmap] and 
                    kwargs[x].dtype in [np.float32, np.float64] else kwargs[x]
                for x in kwargs}
