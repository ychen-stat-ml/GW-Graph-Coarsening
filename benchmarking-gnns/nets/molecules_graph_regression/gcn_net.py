...
# TODO: embedding -> linear
class CoarsenedGCNNet(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        num_atom_type = net_params['num_atom_type']
        num_bond_type = net_params['num_bond_type']
        ...
        self.layers = nn.ModuleList([GCNLayer(hidden_dim, hidden_dim, F.relu,
                    dropout, self.batch_norm, self.residual, norm='none') 
    ...
    def forward(self, g, h, e):
        ...
        for conv in self.layers:
            h = conv(g, h, edge_weight=g.edata['weight'])
        g.ndata['h'] = h

        # print(h.shape, g.ndata['w'].shape)
        ...
        if self.readout == "sum":
            ...
            # weighted mean for graph coarsening
