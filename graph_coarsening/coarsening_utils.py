...
def coarsen(
    ...
    """
    # To compare with other methods, r is now n_target
    # r = np.clip(r, 0, 0.999)
    G0 = G
    N = G.N
    K = min(K, N-1)
    ...
    for level in range(1, max_levels + 1):
        ...
        if "variation" in method:
            ...
            if level == 1:
                ...
                d[mask] = 1
                # dinvsqrt = d ** (-1 / 2)
                dinvsqrt = np.where(d > 0, 1. / np.sqrt(d), 0)
                dinvsqrt[mask] = 0
            ...
            if algorithm == "optimal":
                ...
                # heavy edge matching is actually greedy matching: 
                # the weights will be sorted in matching_greedy
...
def contract_variation_edges(G, A=None, K=10, r=0.5, algorithm="greedy"):
    ...
    # edges = np.array(G.get_edge_list()[0:2])
    tmp_edge_list = list(G.get_edge_list())
    tmp_edge_list[2] = tmp_edge_list[2].reshape(-1)
    edges = np.array(tmp_edge_list)
    # print(edges)
...
def get_proximity_measure(G, name, K=10):
    ...
    # v_in.shape, v_out.shape, weights.shape ((3131,), (3131,), (3131,))

    edges = np.array(G.get_edge_list()[0:2]) # shape: 2 x e
    # weights = np.array(G.get_edge_list()[2]) # e=1, then shape = (), a scalar
    weights = G.get_edge_list()[2].reshape(-1) # shape: (e, )
    ...
    if name == "heavy_edge":
        wmax = np.array(np.max(W, 0).todense())[0] + 1e-5

        # print(edges.shape, weights.shape, proximity.shape, wmax.shape)
        # print(edges, weights, proximity, wmax)
